...
**Re-read the feedback before the summary.** Run `scripts/fetch.ts` again. The pass took time, and a reviewer may have commented during it. On local feedback, read the file again for the same reason. Also check whether the user has said anything since the invocation that changes the ask. Anything the first read missed goes through the same decide, reply, vote and resolve loop. Then query once more. A thread you have just answered drops out of the next read, because your reply is the last comment on it. Write the summary only when a fresh query comes back with nothing left to act on.
